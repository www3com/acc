...
import {clear} from '@/components/session';
import {
    DownOutlined,
    SettingOutlined,
    UserOutlined,
...
const rightContent = ({layoutStore}: any) => {
    ...
    let selectedLedger = {id: "", name: ""};
    const renderItems = (): any => {
        let items: any = [];
        for (const ledger of layoutStore.ledgers) {
            if (ledger.selected == 1) {
    ...
    return (
        <Space split={<Divider type='vertical'/>} size={0}>
            <Dropdown
                menu={{items: renderItems(), selectable: true, defaultSelectedKeys: [selectedLedger.id.toString()]}}
                dropdownRender={(menu) => (
            ...
            >
                <a style={{color: 'black'}}><PayCircleOutlined/> {selectedLedger.name} <DownOutlined/></a>
            </Dropdown>
